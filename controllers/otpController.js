...
const validateOtp = asyncHandler(async (req, res, next) => {
  const { name, email, password, otp, process: processName } = req.body;
  if (!email || !otp) {
    res.status(400);
  ...
  if (record) {
    ...
    if (processName === "register") {
      data.password = password;
    }

    if (processName === "auth_current" || processName === "auth_new") {
